...
class WaymoDataset(Dataset):
    ...
    def read_json(self):
        ...
        if self.split == "test" or self.split=="compose":  # test stage use the data in train.json
            ...
            with open(os.path.join(self.root_dir, f'json/{self.split}.json'), 'r') as fp:
                self.meta = json.load(fp)
            with open(os.path.join(self.root_dir, f'json/split_block_{self.split}.json'), 'r') as fp:  # 每一块的情况
                self.block_split_info = json.load(fp)
